...
async function scanGithubLibrary(cfg, onProgress) {
  ...
  // Camino rápido: si existe music/library.json (pre-generado a mano o con un
  // script), lo usamos directo y no gastamos nada de la cuota de la API.
  onProgress && onProgress("Buscando library.json…");
  const fromJson = await tryLoadLibraryJson(cfg);
...
/**
 * Intenta leer music/library.json directo de raw.githubusercontent.com
 * (no consume cuota de la API de contenidos). Formato esperado:
 * { "playlists": [ { "name": "Rock 80s", "path": "music/Rock 80s",
 *                    "tracks": ["01 Song.mp3", "01 Song.lrc", ...] } ] }
 * Si no existe o no tiene el formato correcto, devuelve null y la app
 * cae de vuelta al escaneo en vivo por la API.
 */
async function tryLoadLibraryJson(cfg) {
  try {
    const res = await fetch(rawUrl(cfg, `${cfg.musicPath}/library.json`));
    if (!res.ok) return null;
    const data = await res.json();
    if (!data || !Array.isArray(data.playlists)) return null;

    return data.playlists.map((p) => {
      const files = p.tracks || [];
      const audioNames = files.filter(isAudioFile);
      const lyricNames = files.filter(n => /\.(lrc|txt)$/i.test(n));
      const lyricsByBase = new Map(lyricNames.map(n => [basenameNoExt(n), n]));

      const tracks = audioNames.map((filename) => {
        const lyricMatch = lyricsByBase.get(basenameNoExt(filename));
        return {
          id: `${p.path}/${filename}`,
          filename,
          url: rawUrl(cfg, `${p.path}/${filename}`),
          lyricsUrl: lyricMatch ? rawUrl(cfg, `${p.path}/${lyricMatch}`) : null,
          title: null, artist: null, album: null, cover: null, duration: null, lyrics: null,
          looksEncoded: /%[0-9A-Fa-f]{2}/.test(filename),
        };
      });
      return { id: p.path, name: p.name, source: "github", path: p.path, tracks };
